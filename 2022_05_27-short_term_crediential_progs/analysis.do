...
global working_directory= "C:\Users\lrestrepo\Documents\Github_repos\The-Learning-Curve\2022_05_27-short_term_crediential_progs"
...
global intermediate = "${data}\\intermediate_data"
global final = "${data}\\final_data"
global vis = "${final}\\vis"

cap n unzipfile "${working_directory}\data.zip"
...
save "${intermediate}\\score_1", replace
...
save "${intermediate}\\score_1_cert", replace
...
save "${intermediate}\\score_2", replace

keep if credlev == 1

gen year = 2

save "${intermediate}\\score_2_cert", replace
...
save "${intermediate}\\score_3", replace

keep if credlev == 1

gen year = 3

save "${intermediate}\\score_3_cert", replace

clear

use "${intermediate}\\score_1_cert"
append using "${intermediate}\\score_2_cert"
append using "${intermediate}\\score_3_cert"
...
save "${intermediate}\\score_cert_panel", replace
...
save "${intermediate}\\scorecard_cert_collapsed", replace
...
save "${final}\FOIA_final1", replace
*********************************************;

* Merging FOIA and scorecard data;

use "${intermediate}\\scorecard_cert_collapsed", clear

merge 1:m opeid cip_4 using "${final}\\FOIA_final1"

keep if _merge ==3 & year == 3

save "${final}\\FOIA_score_merged_final", replace

* Getting counts and summary stats;
tab disapprovalreason
sum completion_recent, detail
sum placement_recent, detail
count if disapprovalreason==""
count if disapprovalreason=="Program Does Not Meet 70/70 Requirements"
count if disapprovalreason==""|disapprovalreason=="Program Does Not Meet 70/70 Requirements"
count if earn_mdn_hi_2yr!=.&(disapprovalreason==""|disapprovalreason=="Program Does Not Meet 70/70 Requirements")
unique programname if earn_mdn_hi_2yr!=.

replace disapprovalreason = "Fails 70/70" if disapprovalreason == "Program Does Not Meet 70/70 Requirements"
replace disapprovalreason = "Pass" if disapprovalreason == ""

* Creating final scatter plot of rates;
preserve
//keep if disapprovalreason == "Program Does Not Meet 70/70 Requirements"
...
graph export "${vis}\graph_1_completion.png", as(png) height(1000) replace
restore

preserve
keep if disapprovalreason == "Fails 70/70"|disapprovalreason == "Pass"
graph bar (mean) earn_mdn_hi_2yr, over(disapprovalreason) bar(1, color("22 150 210")) bar(2, color("22 150 210")) blabel(bar, color("0 0 0") format(%6.2f)) graphregion(color(white)) ylabel(,angle(0)) bgcolor(white) ytitle("")
graph export "${vis}\graph_2_median_earnings.png", as(png) height(1000) replace
restore

* Getting numbers for figure 3 (excluded but counts mentioned in text);

preserve
keep if disapprovalreason == "Fails 70/70"|disapprovalreason == "Pass"
replace bbrr2_fed_comp_noprog = bbrr2_fed_comp_noprog*100
graph bar (mean) bbrr2_fed_comp_noprog, over(disapprovalreason) bar(1, color("22 150 210")) bar(2, color("22 150 210")) blabel(bar, color("0 0 0") format(%2.1f)) ylabel(,angle(0)) graphregion(color(white)) bgcolor(white) ytitle("")
...
* Getting numbers for figure 4 (excluded but counts mentioned in text);
preserve
keep if disapprovalreason == "Fails 70/70"|disapprovalreason == "Pass"
replace ipedscount1 = "." if ipedscount1 == "NULL"
replace ipedscount2 = "." if ipedscount2 == "NULL"
destring earn_cntover150_hi_1yr earn_cntover150_hi_1yr ipedscount1 ipedscount2, replace
collapse (sum) earn_cntover150_hi_1yr earn_cntover150_hi_2yr ipedscount1 ipedscount2, by(disapprovalreason)
gen earn_percover150 = (earn_cntover150_hi_1yr+earn_cntover150_hi_2yr)*100/(ipedscount1+ipedscount2)
graph bar (mean) earn_percover150, over(disapprovalreason) bar(1, color("22 150 210")) bar(2, color("22 150 210")) blabel(bar, color("0 0 0") format(%2.1f)) ylabel(,angle(0)) graphregion(color(white)) bgcolor(white) ytitle("")
local nb=`.Graph.plotregion1.barlabels.arrnels'
forval i=1/`nb' {
  di "`.Graph.plotregion1.barlabels[`i'].text[1]'"
  .Graph.plotregion1.barlabels[`i'].text[1]="`.Graph.plotregion1.barlabels[`i'].text[1]'%"
...
.Graph.drawgraph
graph export "${vis}\graph_4_.png", as(png) height(1000) replace
restore

tab cip_4 if earn_mdn_hi_2yr < 22000

* Looking into which programs pass/fail
count if cip_4 == "1204" & disapprovalreason == "Pass" & earn_mdn_hi_2yr !=.
count if cip_4 == "1204" & disapprovalreason == "Fails 70/70" & earn_mdn_hi_2yr !=.
tab cip_4 if disapprovalreason == "Fails 70/70"
tab cip_4 if disapprovalreason == "Pass"
...
export excel using "${final}\\urban_dta_final", replace
