...
//"Spending by these organizations represents an average extra 3% on top of allocated state and local non-personnel, discretionary funds, and the very largest contribute an additional 20%"
gen shareNonPers = (1-propexp_salben)*pp_total_raw_IL
gen shareDiscretion = Org_PPE/shareNonPers
sum shareDiscretion, d

//"While just over 15% of K-12 public schools in Illinois have a tax-filing fundraising group"
count
local numSchools = r(N)
count if hasPTOOrg
local numSchoolsWithOrgs = r(N)
di `numSchoolsWithOrgs'/`numSchools'
...
sort shareWhite
br ncessch shareWhite exps Org_PPE ein if lea_name == "North Shore SD 112" & school_level == 1
di 49624+155305
di (49624+155305)/2
