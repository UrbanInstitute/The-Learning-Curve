/// CLASS SIZE EQUITY ESSAY #2 - AM, MC, and JC /// 

// Only line that needs to be edited to run normally
glo main "********"
...
gen treated1 = (AverageClassSize > cap) & !missing(AverageClassSize) // this one is our main interest for compliance. we do NOT use treated2 for any of the tables.
gen treated2 = (MaximumClassSize > cap) & !missing(MaximumClassSize) //
...
cap n copy "https://infohub.nyced.org/docs/default-source/default-document-library/demographic-snapshot-2018-19-to-2022-23-(public).xlsx" ///
	"${raw}/demo_data.xlsx"
import excel "${raw}/demo_data.xlsx", sheet("School") firstrow

keep if Year=="2022-23"
...
replace Poverty = "." if Poverty == "Above 95%"	
destring Poverty, replace

replace EconomicNeedIndex=".97" if EconomicNeedIndex=="Above 95%"
destring EconomicNeedIndex, replace

// removing everything but K-5 schools

drop Grade3K GradePKHalfDayFullDay Grade6 Grade7 Grade8 Grade9 Grade10 Grade11 Grade12

save "${int}/School level demographics.dta", replace
...
gen total_classes_needed = ict_needed + gen_ed_needed // Total number of classes in school-grade

label variable sped_count "SPED students in grade"
label variable non_sped_count "GenEd students in grade"
label variable max_sped_count "Max SPED students in ICT classes (calc)"
label variable max_non_sped_count "Max GenEd students in ICT classes"
label variable ict_needed "Number of ICT classrooms needed (calc)"
label variable ict_with_max_sped_students "Number of ICT classrooms with max SPED students"
label variable sped_in_last_ict_class "Number of SPED students in last ICT classroom"
label variable gen_ed_in_max_sped_classes "Number of GenEd students in ICT classes with max SPED students"
label variable gen_ed_in_last_ict_class "Number of GenEd students in last ICT classroom"
label variable gen_ed_left_over "Number of GenEd students that need GenEd classrooms"
label variable gen_ed_needed "Number of GenEd classrooms needed in grade"
label variable total_classes_needed "Number of total classrooms needed in grade"

// calculating # of new teachers needed:
rename NumberofClasses num_class0
ren AverageClassSize avg_cs0
gen num_class1 = total_classes_needed
gen avg_cs1 = num_total/num_class1

foreach v in 1 {
	gen new_teacher`v'=num_class`v'-num_class0
	replace new_teacher`v' = new_teacher`v' *2 if ProgramType == "ICT" //adding additional teacher for ICT
    
}

collapse (rawsum) num_class0 num_class1 new_teacher1 num_total (mean) avg_cs0 avg_cs1 [fw=num_total], by(DBN)
...
order SchoolName Year, after(DBN)
order avg_cs0 avg_cs1, after(Year)
order num_class0 num_class1, after(avg_cs1)
order new_teacher1, after(num_class1)
order SchoolType LowestGrade HighestGrade, after(Year)
order num_total, after (HighestGrade)

// changing negative values for added teachers to missing
...
// calculating baseline added funding
...
export excel using "${fin}/Class Size Implementation V2", sheet("Per Pupil Funding Baseline", modify) firstrow(variables)
...
order race
export excel using "${fin}/Class Size Implementation V2", sheet("Per Pupil Funding Race", modify) firstrow(variables)
...
order povertystatus
export excel using "${fin}/Class Size Implementation V2", sheet("Per Pupil Funding Poverty", modify) firstrow(variables) 
...
order ENI
export excel using "${fin}/Class Size Implementation V2", sheet("Per Pupil Funding ENI", modify) firstrow(variables)
...
export excel using "${fin}/Class Size Implementation V2", sheet("Total by School", modify) firstrow(variables)

// CLASS SIZE IMPLEMENTATION NUMBERS BY STUDENT GROUPS

// baseline 

use "${int}/Funding Analysis File", clear 
collapse avg_cs0 avg_cs1 [fw=num_total]
export excel using "${fin}/Class Size Implementation V2", sheet("Class Size Baseline", modify) firstrow(variables)

// by race
foreach g in Black Hispanic MultiRacial NativeAmerican White MissingRaceEthnicityData {
use "${int}/Funding Analysis File", clear
collapse avg_cs0 avg_cs1 (rawsum) `g' [fw=`g']
gen race = "`g'"
save "${int}/temp_`g'", replace
}
use "${int}/Funding Analysis File", clear
collapse avg_cs0 avg_cs1 (rawsum) Asian [fw=Asian]
gen race = "Asian"
foreach g in Black Hispanic MultiRacial NativeAmerican White MissingRaceEthnicityData {
	append using "${int}/temp_`g'"
	erase "${int}/temp_`g'.dta"
...
order race
export excel using "${fin}/Class Size Implementation V2", sheet("Class Sizes by Race", modify) firstrow(variables) 
...
use "${int}/Funding Analysis File", clear
collapse avg_cs0 avg_cs1 (rawsum) ENI_1_rounded [fw=ENI_1_rounded]
gen ENI = "ENI_1_rounded"
foreach g in ENI_2_rounded ENI_3_rounded ENI_4_rounded {
	append using "${int}/temp_`g'"
	erase "${int}/temp_`g'.dta"
...
order ENI
export excel using "${fin}/Class Size Implementation V2", sheet("Class Sizes by ENI", modify) firstrow(variables) 
...
order race
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("Baseline FSF by Race", modify) firstrow(variables) 
...
order ENI
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("Baseline FSF by ENI", modify) firstrow(variables)
...
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("Baseline Weighted Supplements", modify) firstrow(variables)

use "${int}/Funding Analysis File", clear
collapse fsf_supplement_1_weighted fsf_supplement_2_weighted fsf_supplement_3_weighted fsf_supplement_4_weighted fsf_supplement_5_weighted shortfall_1 shortfall_2 shortfall_3 shortfall_4 shortfall_5 shortfall_1_above_0 shortfall_2_above_0 shortfall_3_above_0 shortfall_4_above_0 shortfall_5_above_0 (rawsum) num_total [fw=num_total], by(SchoolName)
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("Baseline by School", modify) firstrow(variables)
...
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("Baseline without fringe", modify) firstrow(variables)
...
order race
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("By Race", modify) firstrow(variables) 
...
order ENI
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("Supplements and Shortfalls_ENI", modify) firstrow(variables) 
...
order povertystatus
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("by Poverty", modify) firstrow(variables) 
...
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("Sum of weighted supplements", modify) firstrow(variables) 
...
export excel using "${fin}/FSF Supplements and Shortfalls V2", sheet("sum of shortfall if over 0", modify) firstrow(variables) 
...
// increasing cap for ICT: 

replace cap = (cap * 1.25) if ProgramType == "ICT"
...
collapse (rawsum) num_class0 num_class1 new_teacher1 num_total cap (mean) avg_cs0 avg_cs1 [fw=num_total], by(DBN)
merge 1:1 DBN using "${int}/merged_file"
...
// new ICT cap
collapse TotalSchoolFundingperPupil v1_added_per_pupil_cost_district added_per_pupil_cost_no_fringe (rawsum) num_total [fw=num_total]
export excel using "${fin}/Class Size Implementation V2", sheet("Increased ICT cap", modify) firstrow(variables)